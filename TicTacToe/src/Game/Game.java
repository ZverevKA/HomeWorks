...
public class Game extends JFrame {
    ...
    public void start(Player player0, Player player1, GameField gameField){
        ...
        if (gameMode == BotVSBot) {
            while (gameField.getGameStatus() == gameField.EMPTY) {
                while (!(gameField.playerStep(player0)));
                window.repaint();
                if (gameField.getGameStatus() != gameField.EMPTY) {
                    break;
                }
                while (true){
        ...
        if (gameMode == HumanVSBot){
            window.addMouseListener(new MouseListener() {
                ...
                public void mouseClicked(MouseEvent e) {
                    if ((gameField.getWhoseTurn() == gameField.PLAYER0) & (gameField.getGameStatus() == gameField.EMPTY)){
                        player0.x = e.getX() / window.CELL_SIZE;
                        player0.y = e.getY() / window.CELL_SIZE;
                        gameField.playerStep(player0);
                        window.repaint();
                        if ((gameField.getWhoseTurn() == gameField.PLAYER1) & (gameField.getGameStatus() == gameField.EMPTY)) {
