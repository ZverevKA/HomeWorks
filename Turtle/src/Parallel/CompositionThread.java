...
public class CompositionThread extends Thread {
    ...
    private ConvertableToCompositionable array[];
    private final int from;
    private final int to;
    private Compositionable total;
    public CompositionThread(int myID, Message[][] msgPool, int procs, ConvertableToCompositionable array[]){
        this.myID = myID;
    ...
    @Override
    public void run() {
        total = array[from].ConvertToCompositionable(array[from]);
        for (int i = from + 1; i < to; i++){
            Compositionable composition = (Compositionable) total.composition(total, array[i].ConvertToCompositionable(array[i]));
            total = composition;
        }
